...
pub fn validate_timezone(s: &str) -> Result<(), PrefError> {
    chrono_tz::Tz::from_str(s).map(|_| ()).map_err(|_| PrefError::TimezoneUnknown)
}
...
mod tests {
    ...
    fn time_format_round_trips_through_db_and_json() {
        for f in [TimeFormat::Auto, TimeFormat::TwelveHour, TimeFormat::TwentyFourHour] {
    ...
    fn locale_accepts_basic_shapes() {
        for ok in ["en", "fr", "de", "en-US", "pt-BR", "zh-Hans", "zh-Hans-CN", "es-419"] {
    ...
    fn timezone_accepts_canonical_iana_names() {
        for ok in ["UTC", "Europe/Nicosia", "America/Argentina/Ushuaia", "Asia/Tokyo"] {
    ...
    fn timezone_rejects_unknown() {
        for bad in ["", "Mars/Olympus_Mons", "europe/nicosia", "Europe-Nicosia", "PST"] {
            assert!(validate_timezone(bad).is_err(), "{bad:?} should be rejected");
        }
    }
}
