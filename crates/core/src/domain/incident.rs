...
pub fn next_state(
    ...
    use IncidentTransition::{Acknowledge, AutoResolve, Reopen, Resolve};
    let err = || TransitionError { from: current, transition };
...
mod transition_tests {
    ...
    fn resolve_from_any_open_state_and_idempotent() {
        for from in [IncidentState::Triggered, IncidentState::Acknowledged, IncidentState::Resolved]
        {
            assert_eq!(next_state(from, IncidentTransition::Resolve), Ok(IncidentState::Resolved));
            assert_eq!(
...
mod tests {
    ...
    #[expect(
        clippy::float_cmp,
        reason = "exact float equality is intentional for edge-case assertions"
    )]
