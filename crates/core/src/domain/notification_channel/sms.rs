...
impl TransportConfig for SmsConfig {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if !is_sender_id(self.from()) {
            return Err("sender must be an E.164 number, sender id, or messaging-service id".into());
        }
        match self {
            ...
            Self::Telnyx { api_key, messaging_profile_id, .. } => {
                if !is_token(api_key) {
            ...
            Self::Sinch { service_plan_id, api_token, region, .. } => {
