...
const fn default_step_delay() -> i32 {
    300
}
...
pub enum EscalationDecision {
    ...
    /// `delay_secs` from now.
    Page { level: i32, round: i32, delay_secs: i32 },
...
pub fn next_step(
    ...
    match steps.iter().find(|s| s.level > level) {
        Some(next) => {
            EscalationDecision::Page { level: next.level, round, delay_secs: next.delay_secs }
        }
...
/// Per-incident escalation progress. The engine reads this on each tick
/// to decide whether to advance the ladder.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, ToSchema)]
pub struct IncidentEscalationState {
    /// The incident this state tracks.
    pub incident_id: Uuid,
    /// The policy being walked.
    pub policy_id: Uuid,
    /// Last level paged (0 = nothing paged yet). `next_step` advances from
    /// here.
    pub current_level: i32,
    /// Repeat round (0 = first walk through the ladder).
    pub current_round: i32,
    /// When the engine last issued a page for this incident.
    pub last_paged_at: DateTime<Utc>,
    /// When the engine should next consider advancing / re-paging.
    pub next_check_at: DateTime<Utc>,
    /// True once acknowledged — stops further paging.
    pub acked: bool,
}
...
mod tests {
    ...
    fn step(level: i32, delay_secs: i32) -> EscalationStep {
        EscalationStep { id: Uuid::nil(), level, delay_secs, targets: vec![] }
    }
    ...
    fn initial_tick_pages_the_first_level() {
        ...
        assert_eq!(
            ...
            EscalationDecision::Page { level: 1, round: 0, delay_secs: 300 }
        );
    }
    ...
    fn advances_to_the_next_higher_level() {
        ...
        assert_eq!(
            ...
            EscalationDecision::Page { level: 2, round: 0, delay_secs: 600 }
        );
        assert_eq!(
            next_step(&steps, 0, 2, 0),
            EscalationDecision::Page { level: 3, round: 0, delay_secs: 900 }
        );
    }
    ...
    fn last_level_with_repeat_wraps_to_first_and_bumps_round() {
        ...
        assert_eq!(
            ...
            EscalationDecision::Page { level: 1, round: 1, delay_secs: 300 }
        );
        assert_eq!(
            next_step(&steps, 2, 2, 1),
            EscalationDecision::Page { level: 1, round: 2, delay_secs: 300 }
        );
    ...
    fn non_contiguous_levels_advance_to_the_next_present_one() {
        ...
        assert_eq!(
            next_step(&steps, 0, 1, 0),
            EscalationDecision::Page { level: 5, round: 0, delay_secs: 120 }
        );
        assert_eq!(
            next_step(&steps, 0, 5, 0),
            EscalationDecision::Page { level: 9, round: 0, delay_secs: 180 }
        );
