...
fn fmt_days_label(kind: &str, days: i64, name: &str) -> String {
    match days.cmp(&0) {
        std::cmp::Ordering::Less => format!("{kind} expired {} days ago · {name}", -days),
        std::cmp::Ordering::Equal => format!("{kind} expires today · {name}"),
        std::cmp::Ordering::Greater => format!("{kind} expires in {days} days · {name}"),
    }
}
...
mod tests {
    ...
    fn expands_terse_transport_codes() {
        assert_eq!(humanize_check_error("no response"), "connected, but no response (timed out)");
        assert_eq!(humanize_check_error("tls"), "TLS handshake failed");
        assert_eq!(humanize_check_error("connect timeout"), "couldn't connect (timed out)");
    }

    #[test]
    fn de_machines_dns_reasons() {
        assert_eq!(humanize_check_error("dns: domain not found"), "domain not found (DNS)");
        assert_eq!(humanize_check_error("dns: lookup timed out"), "DNS lookup timed out");
    }

    #[test]
    fn formats_tls_cert_json() {
        let raw = r#"{"days_remaining":12,"subject_common_name":"example.com"}"#;
        assert_eq!(humanize_check_error(raw), "cert expires in 12 days · example.com");
    }

    #[test]
    fn formats_domain_expiry_json() {
        let raw = r#"{"days_remaining":-3,"domain":"example.com"}"#;
        assert_eq!(humanize_check_error(raw), "domain expired 3 days ago · example.com");
    }
    ...
    fn passes_through_clean_reasons_and_unknown() {
        assert_eq!(humanize_check_error("connection refused"), "connection refused");
        assert_eq!(humanize_check_error("certificate expired"), "certificate expired");
    }
    ...
    fn served_stale_without_renderable_is_generic() {
        assert_eq!(humanize_check_error(SERVED_STALE_PREFIX), "using last known result");
        assert_eq!(
