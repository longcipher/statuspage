...
/// mirror the DuckDB `INTEGER` type faithfully; the non-zero / arithmetic-safe
/// wrappers belong to the enforcement layer, not the row type. The storage
/// layer owns the row mapping per the domain/storage split used elsewhere
/// in this codebase.
...
/// hard-deleted org takes its quota events with it). The storage layer
/// owns the row mapping per the domain/storage split.
