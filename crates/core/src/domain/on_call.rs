...
pub fn resolve_on_call(
    ...
    let mut covering: Vec<&OnCallOverride> =
        overrides.iter().filter(|o| o.starts_at <= at && at < o.ends_at).collect();
    ...
    let mut ranked: Vec<&OnCallLayer> =
        layers.iter().filter(|l| !l.participants.is_empty()).collect();
    // Top of the stack wins; created_at breaks an order tie deterministically.
    ranked.sort_by(|a, b| b.layer_order.cmp(&a.layer_order).then(a.created_at.cmp(&b.created_at)));
    for layer in ranked {
    ...
    match tz.from_local_datetime(&naive) {
        ...
        LocalResult::None => {
            ...
            for mins in [60, 120, 180, 240] {
                if let Some(dt) =
                    tz.from_local_datetime(&(naive + ChronoDuration::minutes(mins))).earliest()
                {
                    return dt.to_utc();
                }
            }
            let offset = tz.offset_from_utc_datetime(&naive).fix();
            (naive - ChronoDuration::seconds(i64::from(offset.local_minus_utc()))).and_utc()
        }
    }
}
...
mod tests {
    ...
    fn participant(user: UserId, position: i32) -> OnCallParticipant {
        OnCallParticipant { id: Uuid::now_v7(), user_id: user, position }
    }
    ...
    fn daily_handoff_does_not_regress_during_fall_back_hour() {
        ...
        assert_eq!(resolve_on_call(&s, &[l], &[], t("2026-11-01T06:15:00Z")), vec![uid(3)]);
    }
    ...
    fn higher_layer_order_wins_when_stacked() {
        ...
        assert_eq!(resolve_on_call(&s, &[top, base], &[], t("2026-06-01T00:30:00Z")), vec![uid(2)]);
    }
    ...
    fn override_beats_the_rotation_in_its_window() {
        ...
        assert_eq!(resolve_on_call(&s, &[l], &[ov], t("2026-06-01T12:00:00Z")), vec![uid(1)]);
    }
