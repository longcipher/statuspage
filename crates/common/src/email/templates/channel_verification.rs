...
pub fn render(
    ...
    let decline_text =
        decline_url.map(|u| format!(", or block this address: {u}")).unwrap_or_default();
    ...
    let by_org_html =
        org_name.map(|o| format!(" by <strong>{}</strong>", html_escape(o))).unwrap_or_default();
    let decline_html = decline_url
        .map(|u| {
            format!(" Didn't expect this? <a href=\"{}\">Block this address</a>.", html_escape(u))
        })
        .unwrap_or_default();
    ...
    RenderedEmail { subject, text_body, html_body }
}
...
mod tests {
    ...
    fn renders_without_attribution() {
        let r = render("Statuspage", "Ops", "https://app/verify?token=t", 24, None, None);
        assert!(r.text_body.contains("alert channel \"Ops\" on Statuspage"));
