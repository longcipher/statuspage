...
impl std::fmt::Debug for ResendEmailSender {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // api_key is redacted via SecretString; http client internals are not
        // interesting to surface in caller-facing Debug output.
...
impl ResendEmailSender {
    pub fn new(
        ...
        Self { api_key, site_name: site_name.into(), http }
    }
}
...
impl EmailSender for ResendEmailSender {
    async fn send(&self, email: TransactionalEmail) -> EmailResult<MessageId> {
        ...
        let resp =
            self.http.request(req).await.map_err(|e| EmailError::Transport(e.to_string()))?;
