...
pub enum EmailTemplate {
    ...
    MagicLink { url: String, expires_in_minutes: u32, ip_hint: Option<String> },
    ...
    IncidentAlert { body: String, org_name: Option<String>, stop_url: Option<String> },
...
impl EmailTemplate {
    pub fn render(&self, site_name: &str) -> RenderedEmail {
        match self {
            Self::Invitation { org_name, inviter_display, accept_url, decline_url, expires_at } => {
            ...
            Self::MagicLink { url, expires_in_minutes, ip_hint } => templates::magic_link::render(
                site_name,
                url,
                *expires_in_minutes,
                ip_hint.as_deref(),
            ),
            Self::AccountDeletion { scheduled_purge_at } => {
            ...
            Self::IncidentAlert { body, org_name, stop_url } => templates::incident_alert::render(
                site_name,
                body,
                org_name.as_deref(),
                stop_url.as_deref(),
            ),
            Self::SubscriberConfirm { page_name, confirm_url, expires_hours, unsubscribe_url } => {
    ...
    pub fn primary_url(&self) -> Option<&str> {
        match self {
            Self::Invitation { accept_url, .. } => Some(accept_url),
            Self::MagicLink { url, .. } => Some(url),
            Self::AccountDeletion { .. } => None,
            Self::ChannelVerification { verify_url, .. } => Some(verify_url),
            Self::IncidentAlert { .. } => None,
            Self::SubscriberConfirm { confirm_url, .. } => Some(confirm_url),
            Self::SubscriberIncident { incident_url, .. } => Some(incident_url),
            Self::SubscriberMaintenance { page_url, .. } => Some(page_url),
        }
    }
    ...
    pub fn list_unsubscribe_url(&self) -> Option<&str> {
        match self {
            Self::SubscriberIncident { unsubscribe_url, .. }
            | Self::SubscriberMaintenance { unsubscribe_url, .. }
            | Self::SubscriberConfirm { unsubscribe_url, .. } => Some(unsubscribe_url),
            Self::ChannelVerification { decline_url, .. } => decline_url.as_deref(),
            _ => None,
        }
    }
}
...
mod tests {
    ...
    fn incident_alert_withholds_one_click_but_verification_offers_it() {
        ...
        assert_eq!(verify.list_unsubscribe_url(), Some("https://app/alert-channel/stop?c=3&t=4"));
    }
