...
use statuscore::config::TransactionalEmailConfig;

use crate::http_client::outbound::OutboundHttpClient;
...
pub fn build_email_sender(
    ...
    match config.provider.as_str() {
        ...
        other => Err(EmailError::Config(format!("unknown email provider: {other}"))),
    }
}
...
mod tests {
    ...
    fn sample_invitation() -> TransactionalEmail {
        TransactionalEmail {
            ...
            from: EmailAddress::new("no-reply@example.invalid", "Statuspage"),
    ...
    fn invitation_render_includes_action_url_and_org() {
        ...
        let rendered = email.template.render("Statuspage");
        assert!(rendered.subject.contains("Acme"));
        assert!(rendered.subject.contains("Bob"));
        assert!(rendered.text_body.contains("https://example.test/invitations/accept?token=abc"));
    ...
    fn magic_link_render_produces_subject_text_and_html() {
        ...
        assert!(
            rendered.text_body.contains("https://example.test/auth/magic-link/verify?token=xyz")
        );
    ...
    async fn log_only_sender_emits_tracing_and_returns_id() {
        let sender = LogOnlyEmailSender::new("Statuspage [TEST]");
        let id = sender.send(sample_invitation()).await.expect("log-only send");
    ...
    use crate::http_client::outbound::build_outbound_client;

    #[test]
    fn factory_log_provider() {
        let cfg = TransactionalEmailConfig { provider: "log".into(), ..Default::default() };
    ...
    fn factory_memory_provider() {
        let cfg = TransactionalEmailConfig { provider: "memory".into(), ..Default::default() };
        let http = build_outbound_client(crate::security::SsrfGuard::strict());
    ...
    fn factory_resend_requires_api_key() {
        let cfg = TransactionalEmailConfig { provider: "resend".into(), ..Default::default() };
        let http = build_outbound_client(crate::security::SsrfGuard::strict());
    ...
    fn factory_resend_with_api_key_constructs() {
        let cfg = TransactionalEmailConfig {
            ...
            resend: statuscore::config::ResendConfig {
    ...
    fn factory_unknown_provider_errors() {
        let cfg =
            TransactionalEmailConfig { provider: "carrier_pigeon".into(), ..Default::default() };
        let http = build_outbound_client(crate::security::SsrfGuard::strict());
        match build_email_sender(&cfg, &http) {
