...
impl SsrfHttpConnector {
    pub const fn new(guard: SsrfGuard) -> Self {
        Self { guard }
    }
}
...
impl Service<Uri> for SsrfHttpConnector {
    ...
    fn call(&mut self, dst: Uri) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            // less form. Share `security::unbracket` with the SSRF hostname
            // check so both normalise identically.
            let host = crate::security::unbracket(
                dst.host().ok_or_else(|| io::Error::other("missing host in URI"))?,
            )
            .to_string();
            ...
            let addrs: Vec<SocketAddr> =
                resolved.into_iter().filter(|sa| allowed.contains(&sa.ip())).collect();
...
mod tests {
    ...
    async fn rejects_loopback_when_guard_strict() {
        ...
        assert!(err.to_string().contains("blocked"), "expected SSRF block, got: {err}");
    }
    ...
    async fn rejects_link_local_when_guard_strict() {
        ...
        let err = svc.call(dst).await.expect_err("metadata IP must be blocked");
