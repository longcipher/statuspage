...
impl SsrfGuard {
    ...
    pub const fn relaxed_for_tests() -> Self {
        Self::new(true)
    }
    ...
    pub fn filter(&self, host: &str, addrs: Vec<IpAddr>) -> Result<Vec<IpAddr>, SsrfError> {
        ...
        if kept.is_empty() {
            return Err(SsrfError::AllBlocked { host: host.to_string() });
        }
        Ok(kept)
    }
}
...
fn is_blocked_v6(ip: Ipv6Addr) -> bool {
    ...
    // Teredo 2001:0000::/32 (RFC 4380). The client's IPv4 is embedded in the
    // last two 16-bit segments — a public-looking Teredo address can therefore
    // carry 169.254.169.254 (cloud metadata) or 127.0.0.1. Block the entire
    // /32: no legitimate webhook target resolves to a Teredo address.
    if seg[0] == 0x2001 && seg[1] == 0 {
        return true;
    }
    // ISATAP global variants: the EUI-64 interface identifier is
    // `0000:5EFE:<ipv4>` (RFC 5214). A global unicast prefix followed by
    // `::0000:5EFE:xxxx:xxxx` carries an embedded IPv4 in the last two
    // segments. The link-local `fe80::/10` form is already blocked above;
    // this catches the global form, which a public-looking address could
    // otherwise use to reach private v4 infrastructure through an ISATAP
    // router.
    if seg[2] == 0 && seg[3] == 0 && seg[4] == 0 && seg[5] == 0x5efe {
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn blocks_teredo_addresses() {
        // 2001:0000::/32 (RFC 4380). The client IPv4 is in the last two
        // segments. `2001:0::a9fe:a9fe` carries 169.254.169.254 (cloud
        // metadata); `2001:0::7f00:1` carries 127.0.0.1.
        assert!(blocked("2001:0::a9fe:a9fe"));
        assert!(blocked("2001:0::7f00:1"));
        // A Teredo address carrying a *public* v4 is still blocked: the
        // entire /32 is rejected because no legitimate webhook target
        // resolves to a Teredo address.
        assert!(blocked("2001:0::0101:0101"));
    }

    #[test]
    fn blocks_isatap_global_variants() {
        // Global ISATAP: `<prefix>:0000:0000:0000:5EFE:<ipv4>`. The
        // link-local `fe80::/10` form is already blocked by the link-local
        // guard above; this exercises the global form with a non-Teredo
        // prefix so the ISATAP check itself is hit.
        //
        // `2606::5efe:a9fe:a9fe` carries 169.254.169.254 (AWS metadata).
        assert!(blocked("2606::5efe:a9fe:a9fe"));
        // `2606::5efe:7f00:1` carries 127.0.0.1.
        assert!(blocked("2606::5efe:7f00:1"));
        // A global ISATAP address carrying a public v4 is still blocked:
        // the embedded v4 reaches internal infra through the ISATAP router.
        assert!(blocked("2606::5efe:0101:0101"));
    }

    #[test]
    fn guard_with_allow_private_passes_everything() {
        let g = SsrfGuard::new(true);
        assert!(g.check(IpAddr::V4(Ipv4Addr::LOCALHOST)).is_ok());
    ...
    fn guard_filter_errors_when_all_blocked() {
        ...
        match err {
            ...
            SsrfError::BlockedAddress(_) => panic!("wrong variant"),
        }
    }
}
