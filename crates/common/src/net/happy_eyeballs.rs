...
mod tests {
    ...
    async fn errors_on_empty_address_list() {
        let err = connect(vec![], Duration::from_secs(1)).await.expect_err("empty list must error");
    ...
    async fn returns_first_error_when_all_attempts_fail() {
        ...
        let err =
            connect(addrs, Duration::from_secs(2)).await.expect_err("both refused must error");
        // The OS error is ConnectionRefused regardless of family; we mainly
        // assert that *some* error propagated rather than a swallow.
        assert!(!err.to_string().is_empty(), "all-fail must surface a diagnostic");
    }
