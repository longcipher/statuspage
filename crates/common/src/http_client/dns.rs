...
use statuscore::config::DnsConfig;
...
impl std::fmt::Debug for HickoryDnsResolver {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // `TokioResolver` and `Histogram` do not implement `Debug`; expose
        // only the type name so callers get a useful identifier in traces.
...
impl HickoryDnsResolver {
    pub fn new(cfg: &DnsConfig) -> Result<Self> {
        ...
        let name_servers: Vec<NameServerConfig> = cfg
            ...
            .iter()
            .map(|s| {
                // Reuse `parse_resolver_addr` so a server string is parsed
                // identically here and in `build_single_resolver`. The
                // previous `s.split(':').next()` form silently truncated the
                // port on `[::1]:53` (yielding `[`, not `::1`) and lost IPv6
                // scope; `parse_resolver_addr` handles bracketed v6 literals
                // and the `ip:port` form uniformly. We discard the resolved
                // port (hickory's `NameServerConfig::udp` defaults to 53).
                parse_resolver_addr(s)
                    .map(|sock| NameServerConfig::udp(sock.ip()))
                    .wrap_err(format!("invalid dns server ip: {s}"))
            })
        ...
        let resolver =
            Resolver::builder_with_config(resolver_config, TokioRuntimeProvider::default())
                .with_options(opts)
                .build()
                .wrap_err("failed to build hickory resolver")?;

        Ok(Self { inner: Arc::new(resolver), dns_ms: histogram!(names::CHECK_DNS_MS) })
    }

    pub async fn resolve_addrs(&self, host: &str) -> eyre::Result<Vec<IpAddr>> {
...
pub fn build_single_resolver(addr: &str, timeout: Duration) -> eyre::Result<TokioResolver> {
    ...
    Resolver::builder_with_config(cfg, TokioRuntimeProvider::default())
        .with_options(opts)
        .build()
        .wrap_err("failed to build single-server resolver")
}
...
pub fn parse_resolver_addr(s: &str) -> eyre::Result<SocketAddr> {
    if let Ok(sock) = s.parse::<SocketAddr>() {
        return Ok(sock);
    }
    let ip: IpAddr = s.parse().wrap_err(format!("invalid resolver address: {s}"))?;
