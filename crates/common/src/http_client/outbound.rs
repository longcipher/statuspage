...
use crate::http_client::client::install_default_crypto_provider;
use crate::notifier::truncate_bytes;
...
pub async fn post_json_with_headers<T: Serialize + Sync>(
    client: &OutboundHttpClient,
    url: &Url,
    body: &T,
    headers: &std::collections::BTreeMap<String, String>,
) -> Result<()> {
    let payload = serde_json::to_vec(body).wrap_err("serializing request payload")?;
...
pub async fn post_form_with_headers(
    ...
    post_bytes_ct(client, url, "application/x-www-form-urlencoded", payload, headers).await
}

/// POST a `text/plain` body with no extra headers — for subscriber webhook
/// channels that receive the raw payload as the request body. Same SSRF
/// guard, bounded error body, and timeout as the other outbound POSTs.
pub async fn post_text(client: &OutboundHttpClient, url: &Url, payload: Vec<u8>) -> Result<()> {
    post_bytes_ct(client, url, "text/plain", payload, &std::collections::BTreeMap::new()).await
}
...
async fn post_bytes_ct(
    ...
    for (k, v) in headers {
            ...
            (HeaderName::from_bytes(k.as_bytes()), HeaderValue::from_str(v))
        {
            ...
            tracing::warn!(header = %k, "skipping invalid webhook header");
        }
    }
    let req = builder.body(Full::new(Bytes::from(payload))).wrap_err("building request")?;
    ...
    if !status.is_success() {
        ...
        let bytes = limited.collect().await.map(|c| c.to_bytes()).unwrap_or_default();
        let body = String::from_utf8_lossy(&bytes);
        return Err(AppError::Other(eyre::eyre!("endpoint returned {status}: {body}")));
    }
    Ok(())
}
...
pub async fn post_json_capture<T: Serialize + Sync, R: DeserializeOwned>(
    client: &OutboundHttpClient,
    url: &Url,
    body: &T,
) -> Result<R> {
    ...
    if !status.is_success() {
        ...
        return Err(AppError::Other(eyre::eyre!("endpoint returned {status}: {snippet}")));
    }
    serde_json::from_slice(&bytes).map_err(|e| AppError::Other(eyre::eyre!("{url}: {e}")))
}
    ...
    E: std::fmt::Display,
{
    match tokio::time::timeout(REQUEST_TIMEOUT, fut).await {
        Ok(Ok(v)) => Ok(v),
        Ok(Err(e)) => Err(AppError::Other(eyre::eyre!("sending request to {url}: {e}"))),
        Err(_) => {
            Err(AppError::Other(eyre::eyre!("request to {url} exceeded {REQUEST_TIMEOUT:?}")))
        }
    }
}
...
pub async fn get_ok(client: &OutboundHttpClient, url: &Url) -> Result<()> {
    let req =
        Request::get(url.as_str()).body(Full::new(Bytes::new())).wrap_err("building request")?;
    let resp = with_request_timeout(url, client.request(req)).await?;
    let status = resp.status();
    let _ = Limited::new(resp.into_body(), MAX_RESPONSE_BYTES).collect().await;
    if !status.is_success() {
        return Err(AppError::Other(eyre::eyre!("{url} returned {status}")));
    }
    Ok(())
}
...
pub async fn get_json<T: DeserializeOwned>(client: &OutboundHttpClient, url: &Url) -> Result<T> {
    ...
    let resp = with_request_timeout(url, client.request(req)).await?;
    let status = resp.status();
    let collected = Limited::new(resp.into_body(), MAX_RESPONSE_BYTES).collect().await;
    ...
    if !status.is_success() {
        let detail = collected
            ...
            .map(|s| format!(": {s}"))
            .unwrap_or_default();
        return Err(AppError::Other(eyre::eyre!("{url} returned {status}{detail}")));
    }

    let bytes = collected
        .map_err(|e| {
            AppError::Other(eyre::eyre!(
...
fn error_body_summary(bytes: &[u8]) -> String {
    ...
    if let Ok(value) = serde_json::from_slice::<serde_json::Value>(bytes) {
        for key in ["description", "title", "message", "error", "error_description"] {
            ...
            if !text.is_empty() {
                return truncate_bytes(&text, CAP);
            }
        }
    }
    truncate_bytes(String::from_utf8_lossy(bytes).trim(), CAP)
}
...
mod tests {
    ...
    fn error_summary_falls_back_to_raw_text() {
        assert_eq!(error_body_summary(b"plain text failure"), "plain text failure");
    }
