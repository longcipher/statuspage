...
/// Probe paths that bypass the metrics middleware. Caddy / k8s / systemd
/// healthcheck pollers hit these in a tight loop; recording them would
/// dominate every SLO ratio. Inlined here (the original crate lived in
/// `crate::api::handlers::health`) so the middleware stays self-contained
/// without pulling the deleted API layer into the common crate.
...
pub async fn middleware(req: Request, next: Next) -> Response {
    ...
    let route = req
        .extensions()
        .get::<MatchedPath>()
        .map_or_else(|| "<unmatched>".to_string(), |m| m.as_str().to_owned());
